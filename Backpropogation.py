...
for i in range(2000):
    ...
    gradient_hidden_weights = np.dot(input_.reshape(input_neurons, 1),error_terms_hidden.reshape(1, hidden_layer_neurons))
    gradient_output_weights = np.dot(hidden_layer.reshape(hidden_layer_neurons, 1),error_terms_output.reshape(1, output_neurons))
